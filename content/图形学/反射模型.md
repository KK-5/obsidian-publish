...
PBRT中的是为了追求最高精度的渲染，实际许多渲染器都是直接把入射方向和观察方向的中间向量h当做$\omega_m$，所以这里单纯了解一下公式，不深入理解了，并且这里理解难度也很大。
...
$$
p(\omega_i)=D_{\omega_0}(\omega_m)\frac{d\omega_m}{d\omega_i}=\frac{D_{\omega_0}(\omega_m)}{4(\omega_o \cdot\omega_m)}
$$
