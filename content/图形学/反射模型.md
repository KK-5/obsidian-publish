...
按照这个理论，可以得出
$$
\frac{f_r(p,\omega_o,\omega_m)L_i(p,\omega_i)cos\theta_i}{p(\omega_i)}=F(\omega_o,\omega_m)G(\omega_i)L_i(p,\omega_i)
$$
约去Li并移项，可得
$$
f_r(p,\omega_o,\omega_m)=\frac{p(\omega_i)F(\omega_o,\omega_m)G(\omega_i)}{cos\theta_i}
$$
更据上面通过半角向量计算出的pdf，带入后得
$$
f_r(p,\omega_o,\omega_m)=\frac{D_{\omega_o}(\omega_m)F(\omega_o,\omega_m)G(\omega_i)}{4(\omega_o \cdot\omega_m)cos\theta_i}
$$
继续带入上面[[#对可见法线进行采样]]的$D_{\omega_o}(\omega_m)$,得到
$$
f_r(p,\omega_o,\omega_m)=\frac{D(\omega_m)F(\omega_o,\omega_m)G(\omega_i)G(\omega_o)}{4cos\theta_icos\theta_o}
$$
按照[[#阴影-遮蔽函数]]的方法将两个遮蔽函数合成一个，得到最终的BRDF
$$
f_r(p,\omega_o,\omega_m)=\frac{D(\omega_m)F(\omega_o,\omega_m)G(\omega_i,\omega_o)}{4cos\theta_icos\theta_o}
$$
